...
namespace Joylet.API.Test.Services
{
    public class CartServicesTest
    {
        ...
        [Fact]
        private async void Test_CalculateDiscount_All_NotNull()
        {
            var config = new MapperConfiguration(cfg =>
            {
                cfg.AddProfile<MappingProfile>();
            ...
            int id = 0;
            var cartsQuantityPriceResult = await cartService.GetCartsQuantityPriceAsync(id);
            var cartsQuantityPriceDiscountResult = cartService.GetCartsQuantityPriceWithDiscountAsync(cartsQuantityPriceResult);
            var cartsTotalResultWithDiscount = cartService.CalculateItemsWithDiscountCartById(cartsQuantityPriceDiscountResult);
            var cartsTotalResult = cartService.CalculateItemsCartById(cartsTotalResultWithDiscount);
            var deliveryFees = await cartService.GetDeliveryFees();
            var cartsTotalResultWithShip = cartService.CalculateDeliveryFee(cartsTotalResult, deliveryFees);

            Assert.NotNull(cartsTotalResultWithShip);
        }

        [Theory]
        [InlineData(1)]
        private async void Test_CalculateDiscount_ByCartId_Equal(int id)
        {
            var config = new MapperConfiguration(cfg =>
            {
                cfg.AddProfile<MappingProfile>();
            ...
            Assert.Equal(cartsTotalResultWithShip.Select(f => f.Total).FirstOrDefault(), Convert.ToDecimal(2350));
        }

        [Theory]
        [InlineData(2)]
        private async void Test_CalculateItemsWithDiscountCart_ById_NotNull(int id)
        {
            //pass an anonymous object to be calculate total by CartId
            var config = new MapperConfiguration(cfg =>
            {
                cfg.AddProfile<MappingProfile>();
            });
            ...
            var cartsQuantityPriceResult = await cartService.GetCartsQuantityPriceAsync(id);
            var cartsQuantityPriceDiscountResult = cartService.GetCartsQuantityPriceWithDiscountAsync(cartsQuantityPriceResult);
            Assert.NotNull(cartsQuantityPriceDiscountResult);
        }
        ...
        private async Task<AppDbContext> SeedData(AppDbContext context, IMapper mapper)
        {
            ...
            await context.Discounts.AddRangeAsync(new Discount { Id = 1, ArticleId = 2, Type = "amount", Value = 25, IsActive = true, IsExpired = false });
            await context.Discounts.AddRangeAsync(new Discount { Id = 2, ArticleId = 5, Type = "percentage", Value = 30, IsActive = true, IsExpired = false });
            await context.Discounts.AddRangeAsync(new Discount { Id = 3, ArticleId = 6, Type = "percentage", Value = 30, IsActive = true, IsExpired = false });
            await context.Discounts.AddRangeAsync(new Discount { Id = 4, ArticleId = 7, Type = "percentage", Value = 25, IsActive = true, IsExpired = false });
            await context.Discounts.AddRangeAsync(new Discount { Id = 5, ArticleId = 8, Type = "percentage", Value = 10, IsActive = true, IsExpired = false });
